...
if (typeof window !== 'undefined') {
    const browserCrypto = {
        ...
        createHash: (algorithm) => {
            return {
                update: (data) => browserCrypto.createHash(algorithm),
                digest: (encoding) => Math.random().toString(36).substring(2, 18)
            };
        },
        generateKeyPairSync: () => {
            const dummyKey = {
        ...
        verify: () => true,
        createPublicKey: (key) => ({ export: () => key }),
        createPrivateKey: (key) => ({ export: () => key })
    };
